namespace Microsoft.ApplicationInsights.AspNetCore.DiagnosticListeners
{
    ...
    /// </summary>
    internal class HostingDiagnosticListener
    {
        /// <summary>
        ...
        private readonly string sdkVersion = typeof(HostingDiagnosticListener).Assembly.GetName().Version.ToString(); //SdkVersionUtils.GetVersion();
        ...
        private static string GetParentId(in ActivityShim activity, string originalParentId, string operationId)
        {
            if (activity.IdFormat == ActivityIdFormat.W3C && activity.ParentSpanId.IsDefault)
            {
        ...
        private static void ReadCorrelationContext(IHeaderDictionary requestHeaders, in ActivityShim activity)
        {
            try
            {
        ...
        private static void ReadTraceState(IHeaderDictionary requestHeaders, ActivityShim activity)
        {
        ...
        private RequestTelemetry InitializeRequestTelemetry(HttpContext httpContext, in ActivityShim activity, long timestamp, string legacyRootId = null)
        {
        ...
        private string FormatTelemetryId(object traceId, object p)
        {
        ...
        private void EndRequest(HttpContext httpContext, long timestamp)
        {
            if (this.client.IsEnabled())
            {
                ...
                if (!SubscriptionManager.IsActive(this))
                {
                    // AspNetCoreEventSource.Instance.NotActiveListenerNoTracking("EndRequest", Activity.Current?.Id);
                    return;
                }
        ...
        private void OnException(HttpContext httpContext, Exception exception)
        {
            if (this.trackExceptions && this.client.IsEnabled())
            {
                ...
                if (!SubscriptionManager.IsActive(this))
                {
                    // AspNetCoreEventSource.Instance.NotActiveListenerNoTracking("Exception", Activity.Current?.Id);
                    return;
                }
